...
if [[ "${WINESAPOS_BUILD_CHROOT_ONLY}" == "false" ]]; then
    ...
    if [[ "${WINESAPOS_BOOTLOADER}" == "grub" ]]; then
        ...
        printf "\tChecking that GRUB has the command line argument to wait for the 'root' device to initialize..."
        if grep -q "rootwait" "${WINESAPOS_INSTALL_DIR}"/boot/grub/grub.cfg; then
            echo PASS
        else
            winesapos_test_failure
        fi
...
if [[ "${WINESAPOS_DISABLE_KERNEL_UPDATES}" == "true" ]]; then
    ...
    if [[ "${WINESAPOS_DISTRO}" == "manjaro" ]]; then
        if grep -q "IgnorePkg = linux618 linux618-headers linux-fsync-nobara-bin filesystem" "${WINESAPOS_INSTALL_DIR}"/etc/pacman.conf; then
            echo PASS
        else
            winesapos_test_failure
        fi
