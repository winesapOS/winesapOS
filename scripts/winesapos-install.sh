...
SigLevel = Never" >> "${WINESAPOS_INSTALL_DIR}"/etc/pacman.conf
    ...
    if [[ "${WINESAPOS_DISTRO_DETECTED}" == "manjaro" ]]; then
        pacman_install_chroot linux618 linux618-headers
    else
    ...
    if [[ "${WINESAPOS_DISABLE_KERNEL_UPDATES}" == "true" ]]; then
        ...
        if [[ "${WINESAPOS_DISTRO}" == "manjaro" ]]; then
            chroot "${WINESAPOS_INSTALL_DIR}" crudini --set /etc/pacman.conf options IgnorePkg "linux618 linux618-headers linux-fsync-nobara-bin filesystem"
...
if [[ "${WINESAPOS_DE}" == "cinnamon" ]]; then
    ...
    pacman_install_chroot hyprland kitty wofi
...
if [[ "${WINESAPOS_CREATE_DEVICE}" == "true" ]]; then
    chroot "${WINESAPOS_INSTALL_DIR}" pacman -Q | tee ../output/winesapos-packages.txt
