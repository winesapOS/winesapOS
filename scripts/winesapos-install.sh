...
if [[ "${WINESAPOS_ENCRYPT}" == "true" ]]; then
    ...
    sed -i 's/HOOKS=.*/HOOKS=(base microcode udev block keyboard keymap modconf encrypt filesystems resume fsck)/g' "${WINESAPOS_INSTALL_DIR}"/etc/mkinitcpio.conf
else
    sed -i 's/HOOKS=.*/HOOKS=(base microcode udev block keyboard modconf filesystems resume fsck)/g' "${WINESAPOS_INSTALL_DIR}"/etc/mkinitcpio.conf
fi
